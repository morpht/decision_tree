...
class DecisionTree {
  ...
  constructor(id) {
    ...
    if (Array.isArray(steps)) {
      ...
      // Track answer.
      document
          .querySelectorAll("#" + id + " .step .step__answer")
          .forEach(function (answer) {
            if (answer.hasAttribute("href")) {
              answer.addEventListener("click", () => {
                this.trackAnswer(
                    answer.attributes["href"].value.replace("#", ""),
                    answer.hasAttribute("data-answer-path") ? answer.attributes["data-answer-path"].value : false
                );
              });
            }
          }, this);
  ...
  _validateHistory(storage) {
    ...
    var steps = this.config.steps ? this.config.steps : this._loadSteps(this.config.id);
   ...
   */
  _saveStorage() {
    var storage = JSON.parse(localStorage.getItem(this.config.id)) || {};
   ...
   */
  _validation(id) {
    let steps = document.querySelector("#" + id).querySelectorAll(".step");
  ...
  _showSummary() {
    ...
    let display_summary_in_step = document.querySelector("#" + this.config.id + " #" + this.storage.active).hasAttribute('data-show-summary');
    ...
    if (furtherQuestions === 0 || furtherQuestions == null || display_summary_in_step) {
      ...
      if (this.storage.history) {
        ...
        // let summary_element = document.querySelector("#" + this.config.id + " .decision-tree__summary") ? document.querySelector("#" + this.config.id + " .decision-tree__summary") : document.querySelector("#" + this.config.id + " #" + this.storage.active + " .step__info");
        let summary_element = document.querySelector("#" + this.config.id + " .decision-tree__summary");
        if (summary_element && summary_element.nodeType) {
          ...
          if (summary_element.querySelector('.decision-tree__summary_infos')  !== null) {
            ...
            console.warn("Your decision tree with ID " + this.config.id + " does not have element with class decision-tree__summary or step__info.")
        )
        // Filter results by start and stop parameters.
        this.filter("#" + this.config.id + " #" + this.storage.active);
      }
    } else {
  ...
  _cleanHTML() {
    ...
    document.querySelectorAll("#" + this.config.id + " .decision-tree__summary").forEach(
        (element) => {
   ...
   */
  filter(elem) {
    //loop through all answers under current step
    document.querySelectorAll(elem + ' .step__info li').forEach((info) => {
      ...
      if (info.hasAttribute("data-pass-filter")) {
        ...
        let pass = info.attributes["data-pass-filter"].value.split(',');
      ...
      if (info.hasAttribute("data-stop-filter")) {
        //if it has disable filter and that value is in array of answers hide it
        var stop = info.attributes["data-stop-filter"].value.split(',');

        stop.forEach((and) => {
          let result = this.is_in_history(and.trim());
   ...
   */
  trackAttribute(id) {
    var step = document.querySelector("#" + this.config.id + " #" + id);
    // Store the attribute.
    if (step != null) {
  ...
  trackGA(path) {
    if (
        ...
        drupalSettings.google_analytics !== "undefined"
    ) {
      gtag("config", drupalSettings.google_analytics.account, {
        page_path: window.location.href + this.config.id + "/" + path
      });
    } else if (
        typeof ga === "function" &&
        ga.getAll()[0].get("clientId") !== null &&
        ga.getAll()[0].get("trackingId") !== null
    ) {
